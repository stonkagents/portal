 ...
 * from the launch list and the legacy token list, plus the $AGENT mint when the
 * build names one, so each detail path is pre-rendered. The placeholder shell is
 * ALWAYS emitted: CloudFront rewrites /tokens/<mint>/ (and its RSC payload) to
 * it for any mint launched after the build, and the client resolves the mint
 * from the URL.
 */
export async function generateStaticParams() {
  const base = trackerOrigin();
  const agent = AGENT_MINT ? [AGENT_MINT] : [];
  if (!base) {
    return [{ tokenId: 'placeholder' }, ...agent.map(tokenId => ({ tokenId }))];
  }
  ...
  const params = [...new Set([...agent, ...launches, ...listings])].map(tokenId => ({ tokenId }));
