...
/**
 * Nav-optimised animated claw logo.
 * Continuous 4 s spin on an HTML wrapper (compositor-driven); the green drop-shadow
 * sits on a static outer wrapper so the filter is not re-rasterised every frame.
 * `className` lands on the spinning wrapper.
 */
export function ClawLogo({ size = 'md', className }: ClawLogoProps) {
  return (
    <span className="inline-flex shrink-0 drop-shadow-[0_0_6px_rgba(0,255,0,0.4)]" data-testid="claw-logo">
      {/* The spin runs on an HTML element so the compositor animates it; a transform on the
          <svg> itself is main-thread style work every frame. */}
      <span className={cn('inline-flex shrink-0 animate-claw-spin will-change-transform', className)}>
        <svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg" className={cn(sizeMap[size], 'shrink-0')} aria-hidden="true">
          {/* Left claw */}
