...
/**
 * Online mascot — red crab (#FF4D4D) with neon green pixel sunglasses
 */
function ClawOnline() {
  return (
    <g>
...
/**
 * Offline mascot — dimmed red (#CC4040) crab, no sunglasses, sad dot eyes
 */
...
export function ClawMascot({ variant = 'online', animation = 'none', size = 'md', className }: ClawMascotProps) {
  const svg = (
    <svg viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg" className={cn(sizeMap[size], 'block', className)} aria-hidden="true">
      {variant === 'online' ? <ClawOnline /> : <ClawOffline />}
    </svg>
  );
  if (animation === 'none') return svg;
  // Animate an HTML wrapper, not the <svg>: SVG transforms are not compositor-driven and
  // cost a style recalc every frame (measured at ~165/s on the home page).
  return <span className={cn('inline-flex shrink-0 will-change-transform', animationMap[animation])}>{svg}</span>;
}
